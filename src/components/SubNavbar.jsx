...
const SubNavbar = ({ projectView, setProjectView }) => (
  <div className="mb-4 sm:mb-6 -mx-3 sm:mx-0">
    <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100/80 border border-[var(--surface-border)] overflow-x-auto overscroll-x-contain touch-pan-x scrollbar-thin">
      {views.map(({ id, label, icon }) => {
        const active = projectView === id;
        return (
          <button
            key={id}
            type="button"
            onClick={() => setProjectView(id)}
            className={`shrink-0 flex items-center gap-2 px-3 sm:px-4 py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap min-h-[40px] ${
              active

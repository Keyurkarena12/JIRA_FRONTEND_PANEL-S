...
const SubNavbar = ({ projectView, setProjectView }) => {
  return (
    <div className="bg-white border border-gray-200 rounded-lg sm:rounded-none sm:border-x-0 sm:border-t-0 sm:border-b sm:border-gray-200 mb-4 sm:mb-6 -mx-0 sm:mx-0 overflow-hidden">
      <div
        className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 md:px-6 overflow-x-auto overscroll-x-contain scrollbar-thin pb-px"
      ...
      >
        {views.map(({ id, label, icon }) => (
          <button
            ...
            className={`shrink-0 py-2.5 sm:py-3 px-2 sm:px-3 border-b-2 font-medium text-xs sm:text-sm transition-colors whitespace-nowrap ${
              projectView === id

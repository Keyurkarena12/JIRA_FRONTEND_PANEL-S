...
function Dashbord() {
  ...
  // Debug: Log workspaces data to see what structure we're getting
  useEffect(() => {
    console.log("Workspaces data from Redux:", workspaces);
    console.log("Type of workspaces:", typeof workspaces);
    console.log("Is workspaces an array?", Array.isArray(workspaces));
    console.log("Workspaces length:", workspaces?.length);
  }, [workspaces]);

  const handleWorkspaceClick = (workspaceId) => {
    // Navigate to workspace-specific view or open workspace details
    console.log("Opening workspace:", workspaceId);

    navigate(`/workspace/${workspaceId}`);
    // You can add navigation logic here: navigate(`/workspace/${workspaceId}`);
  };

  return (
    <div className="h-[calc(100vh-4rem)] bg-gradient-to-br from-gray-50 via-blue-50 to-white pt-16">
      ...
      <div className="flex">
        {/* LEFT SIDEBAR */}
        <div className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} fixed md:relative md:translate-x-0 z-30 w-64 h-screen bg-white/80 backdrop-blur-xl border-r border-gray-200/50 transition-transform duration-300`}>
            ...
            <div className="space-y-1">
              ...
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                  activeTab === "workspaces" ? "bg-blue-200" : "bg-gray-100"
                }`}>
                  <svg className={`w-4 h-4 ${activeTab === "workspaces" ? "text-blue-600" : "text-gray-600"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                  </svg>
                </div>
                <div>
                  <p className="font-medium">Workspaces</p>
                  <p className="text-xs opacity-70">Your workspaces</p>
                </div>
              </button>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-8 animate-fade-in">
              ...
              <p className="text-blue-600">
                {activeTab === "workspaces" && "Manage and access all your workspaces"}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {loading ? (
                    ...
                    <div className="col-span-full text-center py-12">
                      ...
                      <p className="text-blue-600">You haven't created any workspaces yet.</p>
                    </div>
                  ) : (
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <div className="flex items-center gap-1">
                              {Array.isArray(workspace.members) && workspace.members.slice(0, 3).map((member, index) => (
                                <img
                                  key={index}
                                  src={`https://ui-avatars.com/api/?name=${member.user?.name || member.user?.email || 'User'}&background=6366f1&color=fff&size=24`}
                                  alt={member.user?.name || member.user?.email || 'User'}
                                  className="w-6 h-6 rounded-full border-2 border-white -ml-2 first:ml-0"
                                />
                              ))}
                              {Array.isArray(workspace.members) && workspace.members.length > 3 && (
                                <div className="w-6 h-6 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center -ml-2">
